...
const Delegations = ({
  ...
  const { title, ui, loading, error } = useDelegations(address, {
    page,
  })

  const [isLastPage, setIsLastPage] = useState(false)
  const [contentList, setContentList] = useState<DelegationContent[]>(
    []
  )
...
export const DelegationsModalButton = ({
  address,
}: {
  address: string
}): ReactElement => {
  return (
    <BaseModalButton
      contents={({ closeModal }): ReactElement => (
        <Delegations address={address} closeModal={closeModal} />
      )}
    />
  )
}
