...
interface DefaultAttrs<T>
  extends Pick<FieldAttrs, 'type' | 'autoComplete'> {
  id: keyof T
  name: keyof T
  value: string
}
...
export default <T>(
  ...
  const setValue = (name: keyof T, value: string): void => {
  ...
  return {
    ...
    getDefaultProps: (name): DefaultProps => ({
      element: 'input',
      setValue: (value): void => setValue(name, value),
      error: touched[name] ? errors[name] : '',
    }),
    getDefaultAttrs: (name): DefaultAttrs<T> => ({
      type: 'text' as const,
      id: name,
      name,
