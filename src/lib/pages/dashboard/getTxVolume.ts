...
import { ChartUI, CumulativeType, DisplayCoin } from '../../types'
...
export default (t: TFunction): Props<Result> => ({
  ...
  getValue: (
    results,
    { type, denom, duration }
  ): DisplayCoin | [string, string] | undefined => {
    const result = results.find((r) => r.denom === denom)

    const render = (result: Result): DisplayCoin => {
      const { denom } = result
      const data = result.data.slice(
        duration ? -1 * duration : undefined
      )
      const { txVolume: head } = data.length
        ? data[0]
        : { txVolume: '0' }
      const { txVolume: tail } =
      ...
      const lastDayValue = isCumulative
        ? minus(tail, secondTail)
        : tail
  ...
  getChart: (results, { denom, duration }): ChartUI | undefined => {
    const result = results.find((r) => r.denom === denom)
    return {
      data: (
