...
export interface Props<T = any> {
  ...
  cumulativeLabel?: {
    [CumulativeType.C]: string
    [CumulativeType.P]: string
  }
...
export default <T extends { denom?: string }>(
  props: Props
): ChartCard => {
  ...
  const [type, setType] = useState(
    config?.type?.initial ?? CumulativeType.C
  )
  const [denom, setDenom] = useState(config?.denom?.initial)
  const [duration, setDuration] = useState(
    config?.duration?.initial ?? 0
  )
  ...
  useEffect(() => {
    ...
    const disabled =
      account === AccountType.A && type === CumulativeType.C
  ...
  type Response =
    | T[]
    | ({ [key in CumulativeType]: T[] } & { total?: number })
  const getURL = (): string =>
    typeof url === 'string' ? url : url(filter)
  ...
  const renderFilter = (): ChartFilter => ({
    type: config?.type
      ? {
          value: type,
          set: setType,
          options: [CumulativeType.C, CumulativeType.P].map(
            (value) => ({
              value,
    ...
    duration: {
      ...
      set: (v: string): void => setDuration(Number(v)),
      options: (config?.duration?.list ?? [0, 7, 14, 30]).map(
        (value) => ({
          value: String(value),
          children:
            value === 0
