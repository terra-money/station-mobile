...
import { Deposit, DisplayCoin, FinderFunction } from '../../types'
...
export const convertVote = (
  ...
  const getRatio = (n: string): string =>
    isVoted ? div(n, total) : String(0)
  const getItem = (
    key: string
  ): {
    label: string
  ...
  const sorted = sort(
    (a, b) =>
      a.amount === b.amount ? 0 : lt(a.amount, b.amount) ? 1 : -1,
    list
  )
...
export const calcDepositRatio = ({
  ...
  !totalDeposit.length
    ? String(0)
    : div(
        totalDeposit[0] ? totalDeposit[0].amount : 0,
        minDeposit[0].amount
      )
