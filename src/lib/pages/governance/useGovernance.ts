...
export default ({ status }: { status: string }): GovernancePage => {
    ...
    return Object.assign(
      {
        ...
        title,
        meta: t(
          'Page:Governance:Submitted by {{proposer}} at {{date}}',
          {
            proposer:
              proposer.moniker ??
        ...
        .concat(
          mostVoted
            ? {
                ...
                content: `${mostVoted.label} (${percent(
                  mostVoted.ratio
                )})`,
              }
            : []
  ...
  const render = ({
    ...
    return Object.assign(
      ...
      !proposals.length
        ? {
            message: t(
              'Page:Governance:No proposals here yet. Be the first!'
            ),
          }
  ...
  return Object.assign(
    {
      propose: {
        attrs: { children: t('Page:Governance:New proposal') },
      },
    },
    response,
...
/* helpers */
const formatNS = (nanosecond: string): string =>
