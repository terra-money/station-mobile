...
import { DisplayCoin, User } from '../../types'
...
import {
  ValidatorListHeadings,
  ValidatorListHeading,
} from '../../types'
...
export default (
  ...
  const renderPersonal = ({
    ...
    const myDelegationsFiltered = myDelegations?.filter(
      ({ amountDelegated }) => gte(amountDelegated, 1)
    )

    const undelegationTotal = calcUndelegationTotal(undelegations)

    const getMyDelegationsTable = (
      title: string,
      sortKey: SortKey
    ):
      | {
      ...
      const sorted =
        myDelegationsFiltered?.sort(compareWith(sortKey)) ?? []
    ...
    return {
      withdrawAll: {
        ...
        amounts:
          rewards?.denoms.map((coin) => format.display(coin)) ?? [],
        validators:
  ...
  const renderValidators = (staking: StakingData): StakingUI => {
    ...
    const sorted = validators
      ...
      .sort((validatorA, validatorB) => {
        const a = String(path(prop.split('.'), validatorA) || 0)
        const b = String(path(prop.split('.'), validatorB) || 0)
        const c = asc ? 1 : -1
        const compareString =
          c * (a.toLowerCase() > b.toLowerCase() ? 1 : -1)
  ...
  return Object.assign(
    ...
    response,
    user &&
      response.data && { personal: renderPersonal(response.data) },
...
const compareWith = (key: SortKey) => (
  a: StakingDelegation,
  b: StakingDelegation
): number => toNumber(minus(b[key] ?? '0', a[key] ?? '0'))
