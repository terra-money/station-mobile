...
export const useCalcFee = ():
  | {
      gasPrices: Dictionary<string>
      gasPrice: (denom: string) => string
      feeFromGas: (gas: string, denom: string) => string
      gasFromFee: (fee: string, denom: string) => string
    }
  | undefined => {
  const url = '/v1/txs/gas_prices'
  const { data } = useFCD<Dictionary<string>>({ url })

  const getGasPrice = (denom: string): string => data?.[denom] ?? '0'

  const calcFeeFromGas = (gas: string, denom: string): string =>
    ceil(times(gas, getGasPrice(denom)))

  const calcGasFromFee = (fee: string, denom: string): string =>
    floor(div(fee, getGasPrice(denom)))

  const gasPrices = Object.entries(data ?? {}).reduce<
    Dictionary<string>
  >(
...
type BaseAccountValue = {
  BaseVestingAccount: { BaseAccount: AccountValue }
}
export const getBase = async (from: string): Promise<Base> => {
  ...
  const { data: account } = await fcd.get<Account>(
    `/auth/accounts/${from}`
  )
...
const getValue = ({ result: { value } }: Account): AccountValue =>
  Object.assign(
    ...
    'BaseVestingAccount' in value
      ? value.BaseVestingAccount.BaseAccount
      : value
  )
