...
export const useCoinsFields = (denoms: string[]): CoinFields => {
  ...
  const [coinInputs, setCoinInputs] = useState<CoinInput[]>([
    InitialCoinInput,
  ])
    ...
    groups: coinInputs.map(({ denom, input }, index) => ({
      ...
      button: index
        ? {
            children: '-',
            onClick: (): void =>
              setCoinInputs(remove(index, 1, coinInputs)),
          }
        : {
            children: '+',
            onClick: (): void =>
              setCoinInputs([...coinInputs, InitialCoinInput]),
          },
    })),
    coins: coinInputs
      .filter(({ input }) => !!input)
