...
import { ProposalItemUI, GovernanceParamUI } from '@terra-money/use-native-station'
        ...
        <View style={{
          ...
          <View style={{
            ...
            <View style={{flexDirection:'row'}}>
              <Icon name='info-outline' size={12} color='rgb(32,67,181)' style={{alignSelf:'center'}} />
              <Text style={{fontSize:14, lineHeight:21, color:'rgb(32,67,181)'}}>Parameters</Text>
            </View>
            <TouchableOpacity style={{}} onPress={()=>{
              setParamVisibility(!paramVisibility)
            }}>
              <Icon name={paramVisibility?'expand-less':'expand-more'} size={12} color='rgb(32,67,181)' />
            </TouchableOpacity>
          </View>
          {
            ...
            <View style={{flexDirection:'row', justifyContent:'space-between', marginTop:16, marginBottom:5}}>
              <Text style={styles.ParametersItemText}>{ui?.params[0].title}</Text>
              <Text style={styles.ParametersItemText}>{ui?.params[0].content}</Text>
            </View>
            <View style={{flexDirection:'row', justifyContent:'space-between', marginBottom:5}}>
              <Text style={styles.ParametersItemText}>{ui?.params[1].title}</Text>
              <View style={{flexDirection:'row'}}>
                <Text style={styles.ParametersItemText}>{ui?.params[1].displays?.[0].value}</Text>
                <Text style={styles.ParametersItemText}>&nbsp;</Text>
                <Text style={styles.ParametersItemText}>{ui?.params[1].displays?.[0].unit}</Text>
              </View>
            </View>
            <View style={{flexDirection:'row', justifyContent:'space-between'}}>
              <Text style={styles.ParametersItemText}>{ui?.params[2].title}</Text>
              <Text style={styles.ParametersItemText}>{ui?.params[2].content}</Text>
            </View> 
            </>
          }
        </View>
